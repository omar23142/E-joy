...
export class VideosService {
  ...
  async getOrCreateVideo(dto: CreateVideoDto) {
    ...
    const platformId: string = this.extractPlatformId(originalUrl, platform);
    ...
    try {
      ...
      if (
        ...
        (err.message.includes('unique') || (err as any).code === '23505')
      )
  ...
  async findOne(videoId: number, user: User) {
    ...
    const video = await this.videoRepo
      .createQueryBuilder('video')
      .innerJoin('video.vocabulary', 'voc', 'voc.userId =:userId', {
        userId: user.id,
      })
    ...
    if (!video)
      throw new NotFoundException(
        'this video with this id: ' + videoId + ' and for this user not exist',
      );
    return video;
  }
  ...
  async unlinkVideoFromVocab(userId: number, videoId: number) {
    return await this.vocabRepo.update(
  ...
  async removeForAdmin(videoId: number) {
    ...
    if (!video)
      throw new BadRequestException(
        'this video is not exist be sure from the id ',
      );
  ...
  public async IsVideoExist(VideoId: number) {
    ...
    if (!video) throw new NotFoundException('this video is no longer exist');
    return video;
  }
  ...
  private extractPlatformId(originalUrl: string, platform: string) {
    ...
    switch (platform.toLowerCase()) {
      ...
      case 'vimeo':
        return (
          findId(/(?:vimeo\.com\/|player\.vimeo\.com\/video\/)([0-9]+)/) ||
          originalUrl
        );

      case 'coursera':
        return (
          findId(/coursera\.org\/learn\/[^\/]+\/lecture\/([^\/?#]+)/) ||
          originalUrl
        );

      case 'udemy':
        return (
          findId(/udemy\.com\/course\/[^\/]+\/learn\/lecture\/(\d+)/) ||
          originalUrl
        );
      ...
      case 'instagram':
        ...
        return (
          findId(/instagram\.com\/(?:p|reel|tv)\/([A-Za-z0-9_-]+)/) ||
          originalUrl
        );
      ...
      case 'tiktok':
        ...
        return (
          findId(/tiktok\.com\/@[^\/]+\/video\/(\d+)/) ||
          findId(/vm\.tiktok\.com\/([A-Za-z0-9]+)/) ||
          originalUrl
        );

      default:
        const urlhashed = createHash('md5').update(originalUrl).digest('hex');
        return urlhashed;
    }
  }
}
